<template>
  ...
  <el-container>
    <el-aside>
      ...
      <div class="images">
       <h5 style="color: aliceblue"><img :src="imageUrl" height="40" width="40" @click="connection"/>       &nbsp;&nbsp; {{cid}}
       </h5>
      </div>
        <div class="input">
        <el-input
      ...
      <span class="el-dropdown-link" >
        ...
        <el-tab-pane label="修改头像" >
          <input type="file" name="avatar" accept="image/gif,image/jpeg,image/jpg,image/png"
                 @change="update($event)" ref="avatarInput">
        </el-tab-pane>
    ...
    update (event) {
      ...
      this.$axios.post('/api/uploadImg', param)
        .then(response => {
          ...
          sessionStorage.setItem('images', '/static/' + this.imageUrl)
